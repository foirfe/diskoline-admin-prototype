...
export default function StopForm({ saveStop, stop }) {
  ...
  function handleSubmit(event) {
    ...
    const validForm =
      formData.danish_name &&
      formData.english_name &&
      formData.greenlandic_name; // will return false if one of the properties doesn't have a value
    if (validForm) {
      saveStop(formData);
    } else {
    ...
    <form onSubmit={handleSubmit}>
      <div className="name-inputs">
        ...
        <label>
          ...
          <input
            type="text"
            value={greenlandicName}
            onChange={(e) => setGreenlandicName(e.target.value)}
          />
        </label>
        <label>
          Kode:
